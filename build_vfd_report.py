#!/usr/bin/env python3
"""
SISL VFD Stock Report Generator · v1.5
• Reads three CSVs from ./data (fallback to repo/data)
• Robust header mapping for Model Name, Qty owned, Total cost, 1.27, List Price
• Normalizes model strings (UPPERCASE, no spaces, strip trailing '-1')
        ...
        for col in cols:
            ...
            for pat in patterns:
                if isinstance(pat, str):
                    ...
                    # list of tokens
                    if all(token in low for token in pat):
                        mapping[col] = need
                        break
            if col in mapping:
                break
...
inv = map_cols(
    pd.read_csv(INV_CSV),
    {
        "Model Name": [["model", "name"], "model name", ["material", "name"]],
        "Qty owned":  ["qty owned", ["qty", "owned"], "quantity", "qty"],
...
plist = map_cols(
    ...
    {
        "Model Name": [["model", "name"], "model name", ["material", "name"]],
        "List Price": [["list", "price"], "list price", "price list"]
    }
)

# ─── Normalize model keys everywhere ────────────────
...
def fallback127(m: str):
    ...
    for prefix in ("FR-E820", "FR-E840"):
        v = p127_map.get(f"{prefix}-{suffix}")
        if v is not None:
            return v
    return None
...
inv["SeriesOrder"] = inv["__M__"].apply(series_tag).map(
    {"D": 0, "E": 1, "F": 2, "A": 3, "H": 4}
).fillna(99)
